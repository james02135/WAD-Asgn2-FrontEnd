...
class StubAPI {
    ...
    updateBeer(key, abv, color, review) {
        let index = _.findIndex(this.beers, beers => beers.name === key);
        if (index !== -1) {
        this.beers[index].abv = abv;
        this.beers[index].color = color;
        this.beer[index].review = review;
        return true;
        }
        return false;
    }

    updateBeerReview(key, name, aroma, mouthfeel, color, flavor, comments ) {
        let index = _.findIndex(this.beers, beers => beers.id === key);
        if (index !== -1) {
            this.beers[index].name = name;
            this.beers[index].aroma = aroma;
            this.beers[index].mouthfeel = mouthfeel;
            this.beers[index].color = color;
            this.beers[index].flavor = flavor;
            this.beers[index].comments = comments;
        return true;
        }
        return false;
    }
